---
name: mdoc-cli
description: 通过 Node.js CLI 调用 mdocs 的搜索/读取/创建接口，输出统一 JSON，供本地 agent 使用。
---

# 用途与边界

- 该 skill 用于调用本地 **mdocs** Markdown 知识库的四项能力：关键词搜索、读取文章、创建文章、创建目录。
- CLI 只做 HTTP 薄封装，请求通过 `x-cli-token` header 认证。
- 本 skill 的 CLI 入口固定在 `~/.mdocs-cli/mdoc.mjs`（见下文分发流程）。

# 固定目录

- **仓库地址**：`http://git.ddxq.mobi/xuhuafei/mdocs-cli.git`
- **本机目录（写死）**：`~/.mdocs-cli`
...
```bash
MDOCS_CLI="${HOME}/.mdocs-cli"
REPO_URL="http://git.ddxq.mobi/xuhuafei/mdocs-cli.git"
if [[ -d "${MDOCS_CLI}/.git" ]]; then
  git -C "${MDOCS_CLI}" pull --rebase
elif [[ -e "${MDOCS_CLI}" ]]; then
  echo "error: ${MDOCS_CLI} 存在但不是 git 仓库，请用户处理" >&2
  exit 1
else
  git clone "${REPO_URL}" "${MDOCS_CLI}"
fi
```
...
```bash
node ~/.mdocs-cli/mdocs.mjs create --domain <域ID> --path "笔记/xxx.md" --title "标题" --content "正文"
node ~/.mdocs-cli/mdocs.mjs create --domain <域ID> --path "笔记/xxx.md" --title "标题" --file /tmp/content.md
```

- `--path`：磁盘相对路径（必填），如 `` 方案/笔记.md``
